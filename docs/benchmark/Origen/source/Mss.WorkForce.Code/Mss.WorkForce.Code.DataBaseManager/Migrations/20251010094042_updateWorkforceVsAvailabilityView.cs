...
{
    /// <inheritdoc />
    public partial class updateWorkforceVsAvailabilityView : Migration
    {
        /// <inheritdoc />
...
AS WITH plan AS (
  ...
  SELECT generate_series(case when DATE_PART('hour',min(itemplanning.""InitDate"")) > w.""InitHour"" then floor(w.""InitHour"")::integer
        else floor(DATE_PART('hour',min(itemplanning.""InitDate"")))::integer
        end,
        case when DATE_PART('hour', max(itemplanning.""EndDate"")) > w.""EndHour"" then ceiling(DATE_PART('hour', max(itemplanning.""EndDate"")))::integer
        else ceiling(w.""EndHour"")::integer
        end, 1) as hour, w.""AvailableWorkerId"", w.""Name""
     FROM workerShiftsForLastPlanning w,  itemplanning
...
SELECT h.""Name"",
 h.""When"",
   ...
   left JOIN (select ""Name"", hour AS ""When"",
   COALESCE(EXTRACT(epoch FROM worktime), 0::numeric) AS worktime
   from intervals_merged_grouped )wt
